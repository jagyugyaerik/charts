...
The `maintainers` key in a Helm chart’s Chart.yaml should always be filled in because it documents who is responsible for the chart and how to contact them, which is essential for long‑term maintenance and support. Many CI linting setups and chart repositories expect this metadata and may warn or fail if maintainers are missing, since the field is used in automated checks and review workflows. Providing at least a maintainer name (and optionally email or URL) also improves transparency and community trust, making it easier for users to report bugs, request features, or coordinate security fixes for the chart.
...
maintainers: # (required for helm linter)
  - name: The maintainers name (required for each maintainer)
    email: The maintainers email (optional for each maintainer)
    url: A URL for the maintainer (optional for each maintainer)
icon: A URL to an SVG or PNG image to be used as an icon (recommended for helm lint).
